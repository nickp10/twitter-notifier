...
{
	// This code was taken from: http://jacob4u2.blogspot.com/2009/09/bind-html-to-textblock-with-html-to.html
	// which was in turn taken from MSDN as an example of converting HTML to XAML.
	//
	...
	internal class HtmlParser
	{
		...
		{
			// Create artificial root element to be able to group multiple top-level elements
	...
	{
		public const string DATA_ATTRIBUTE = "data-";

		public const string PRESENTATION_NAMESPACE = _xamlNamespace;

		public const string XAML_PREFIX = "x";
		public const string XAML_NAMESPACE = "http://schemas.microsoft.com/winfx/2006/xaml";

		public const string COLLECTIONS_PREFIX = "col";
		public const string COLLECTIONS_NAMESPACE = "clr-namespace:System.Collections;assembly=mscorlib.dll";

		public const string SYSTEM_PREFIX = "sys";
		public const string SYSTEM_NAMESPACE = "clr-namespace:System;assembly=mscorlib";
		...
		public static string ConvertHtmlToXaml(string htmlString, bool asFlowDocument)
		{
			...
			// Add additional XAML namespaces
			xamlFlowDocumentElement.SetAttribute("xmlns:" + XAML_PREFIX, XAML_NAMESPACE);
			xamlFlowDocumentElement.SetAttribute("xmlns:" + COLLECTIONS_PREFIX, COLLECTIONS_NAMESPACE);
			xamlFlowDocumentElement.SetAttribute("xmlns:" + SYSTEM_PREFIX, SYSTEM_NAMESPACE);
		...
		private static void AddImage(XmlElement xamlParentElement, XmlElement htmlElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
		{
			...
			Hashtable localProperties;
			Hashtable currentProperties = GetElementProperties(htmlElement, inheritedProperties, out localProperties, stylesheet, sourceContext);
			XmlElement xamlElement = xamlParentElement.OwnerDocument.CreateElement(null, HtmlToXamlConverter.Xaml_Image, _xamlNamespace);
			...
			if (src != null)
			{
				if (src.EndsWith(".gif"))
				{
					xamlElement.SetAttribute(Xaml_AnimatedGifSource, Xaml_AnimatedGifNamespace, src);
					xamlElement.SetAttribute(Xaml_AnimatedGifAutoStart, Xaml_AnimatedGifNamespace, bool.TrueString);
				}
				else
				{
			...
			while (propertyEnumerator.MoveNext())
			{
				switch ((string)propertyEnumerator.Key)
				{
					...
					case "display":
						break;
					case DATA_ATTRIBUTE:
						var dataAttributes = propertyEnumerator.Value as Hashtable;
						if (dataAttributes != null && xamlElement != null)
						{
							CreateTagFromDataAttributes(xamlElement, dataAttributes);
						}
						break;
				}
			}
		...
		/// <summary>
		/// Creates a HashTable in the Tag of the XAML element. The key/value pairs are obtained
		/// from the data attributes on the original HTML element.
		/// </summary>
		/// <example>
		///		The following HTML:
		///		<p data-foo="bar" data-baz="qux">My Paragraph</p>
		/// 
		///		Will result in the following XAML:
		///		<Paragraph>
		///			<Paragraph.Tag>
		///				<col:Hashtable>
		///					<sys:String x:Key="foo">bar</sys:String>
		///					<sys:String x:Key="baz">qux</sys:String>
		///				</col:Hashtable>
		///			</Paragraph.Tag>
		///			My Paragraph
		///		</Paragraph>
		/// </example>
		/// <param name="xamlElement">The XAML element to which to add a HashTable to its Tag.</param>
		/// <param name="dataAttributes">A Hashtable that contains the data attributes from the original HTML element.</param>
		private static void CreateTagFromDataAttributes(XmlElement xamlElement, Hashtable dataAttributes)
		{
			Contract.Requires(xamlElement != null);
			Contract.Requires(dataAttributes != null);

			var document = xamlElement.OwnerDocument;
			var hashTableElement = document.CreateElement(COLLECTIONS_PREFIX, "Hashtable", COLLECTIONS_NAMESPACE);

			foreach (DictionaryEntry entry in dataAttributes)
			{
				var keyValueElement = document.CreateElement(SYSTEM_PREFIX, "String", SYSTEM_NAMESPACE);
				keyValueElement.SetAttribute("Key", XAML_NAMESPACE, (string)entry.Key);
				keyValueElement.InnerText = (string)entry.Value;
				hashTableElement.AppendChild(keyValueElement);
			}

			var tagElement = document.CreateElement(xamlElement.LocalName + ".Tag", PRESENTATION_NAMESPACE);
			tagElement.AppendChild(hashTableElement);
			xamlElement.AppendChild(tagElement);
		}
		...
		private static Hashtable GetElementProperties(XmlElement htmlElement, Hashtable inheritedProperties, out Hashtable localProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
		{
			...
			// Add the data attributes to the 'localProperties' only. Data attributes are not inherited and so
			// they should not be added to the 'currentProperties' which gets passed to child elements.
			AddDataAttributes(htmlElement, localProperties);

			return currentProperties;
		}

		/// <summary>
		/// Creates a Hashtable that stores all data attributes defined on the htmlElement
		/// and then stores that Hashtable inside of the localProperties Hashtable.
		/// </summary>
		/// <param name="htmlElement">An HTML element that may contain data attributes.</param>
		/// <param name="localProperties">A Hashtable that stores properties of the htmlElement.</param>
		private static void AddDataAttributes(XmlElement htmlElement, Hashtable localProperties)
		{
			foreach (var attribute in htmlElement.Attributes.OfType<XmlAttribute>().Where(a => a.LocalName.StartsWith(DATA_ATTRIBUTE, StringComparison.OrdinalIgnoreCase)))
			{
				var key = attribute.LocalName.Substring(DATA_ATTRIBUTE.Length);
				if (!string.IsNullOrWhiteSpace(key))
				{
					if (!localProperties.ContainsKey(DATA_ATTRIBUTE))
					{
						localProperties.Add(DATA_ATTRIBUTE, new Hashtable());
					}

					((Hashtable)localProperties[DATA_ATTRIBUTE])[key] = attribute.Value;
				}
			}
		}
		...
		public const string Xaml_AnimatedGifAutoStart = "ImageBehavior.AutoStart";
		public const string Xaml_AnimatedGifSource = "ImageBehavior.AnimatedSource";
		public const string Xaml_AnimatedGifNamespace = "http://wpfanimatedgif.codeplex.com";

		public const string _xamlNamespace = "http://schemas.microsoft.com/winfx/2006/xaml/presentation";
	}
